...
public abstract class Training {
	...
	public NeuralNet train(NeuralNet n) {
		...
		while (this.getEpochs() < n.getMaxEpochs()) {
			...
			for (int i = 0; i < rows; i++) {
				...
				estimatedOutput = this.activationFnc(n.getActivationFnc(), netValue); //-->1
				realOutput = n.getRealOutputSet()[i]; // -->0
	...
	protected double activationFnc(ActivationFncENUM fnc, double value) { // protected
		switch (fnc) {
		case STEP:
	...
	public void printTrainedNetResult(NeuralNet trainedNet) {
		Logger log = Logger.getLogger(Training.class);
		...
		for (int i = 0; i < rows; i++) {
			...
			for (int j = 0; j < cols; j++) {
				...
				log.debug(trainedNet.getTrainSet()[i][j] + "\t");
			}
			...
			log.debug(" NET OUTPUT: "  + estimatedOutput + "\t");
			log.debug(" REAL OUTPUT: " + realOutput + "\t");
